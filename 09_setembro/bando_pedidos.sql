...
CREATE TABLE VENDEDOR ( 
    ...
    CONSTRAINT FK_VENDEDOR_SETOR FOREIGN KEY (CODSETOR) REFERENCES SETOR (CODSETOR) 
...
CREATE TABLE CLIENTE ( 
    ...
    CONSTRAINT FK_CLIENTE_CIDADE FOREIGN KEY (CODCID) REFERENCES CIDADE (CODCID) 
...
CREATE TABLE PEDIDO ( 
    ...
    CONSTRAINT FK_PEDIDO_VENDEDOR FOREIGN KEY (CODVEND) REFERENCES VENDEDOR (CODVEND) 
...
CREATE TABLE ITEMPEDIDO ( 
    ...
    CONSTRAINT PK_ITEMPEDIDO PRIMARY KEY (NUMPED, CODPROD), 

    CONSTRAINT FK_ITEMPEDIDO_PRODUTO FOREIGN KEY (CODPROD) REFERENCES PRODUTO (CODPROD), 

    CONSTRAINT FK_ITEMPEDIDO_PEDIDO FOREIGN KEY (NUMPED) REFERENCES PEDIDO (NUMPED) 
...
select numped from itempedido group by numped order by sum(qtdade) asc limit 1; 

select numped from itempedido group by numped order by sum(qtdade) desc limit 1; 

select codprod from produto where VALOR_UN = (select min(valor_un) from produto); 

select codprod from produto where VALOR_UN = (select max(valor_un) from produto); 
